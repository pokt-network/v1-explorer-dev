...
export const rpcClientRouter = router({
  listValidators: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      return await client.query.postV1QueryValidators({ height: Number(input.height), page, per_page })
    }),
  height: publicProcedure.query(() => client.query.getV1QueryHeight()),
  queryBlock: publicProcedure.input(
    z.object({
      height: z.bigint(),
    }),
  ).query(async ({ input }) => client.query.postV1QueryBlock({ height: Number(input.height) })),
});
